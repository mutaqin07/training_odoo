...
class CourseXlsx(models.AbstractModel):
    _name = 'report.training_odoo.report_course' # report.nama_module.id
    ...
    def generate_xlsx_report(self, workbook, data, obj):
        ...
        sheet.merge_range(0, 0, 0, 1, "Reference", text_top_style) # (start baris, start kolom, end baris, end kolom, text. style)
        ...
        # ============  cara 1 dengan langsung isi dari List  ===========
        # no_list = []
        # session = []
        # partner = []
        # start_date = []
        ...
        for x in obj.session_line:
            ...
            sheet.write(row, 1, x.name or '', text_style)
            sheet.write(row, 2, x.partner_id.name if x.partner_id and x.partner_id.name else '', text_style)
            sheet.write(row, 3, x.start_date.strftime('%d-%m-%Y') if x.start_date else '', text_style)
            sheet.write(row, 4, x.end_date.strftime('%d-%m-%Y') if x.end_date else '', text_style)
            sheet.write(row, 5, x.duration or '', text_style)
            sheet.write(row, 6, x.seats or '', text_style)
            sheet.write(row, 7, x.attendees_count, text_style)
            sheet.write(row, 8, x.taken_seats, text_style)
            sheet.write(row, 9, x.state.capitalize(), text_style)
            no += 1
            total += x.taken_seats
            row += 1
        sheet.write(row, 8, total, text_style)
